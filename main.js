...
var myHashFunction = (randomString) => {
        return randomString.toString.length * Math.exp(2);
};
...
var calculateHash = (index, previousHash, timestamp, data, nonce) => {
    return (
        Math.ceil(myHashFunction(index)  + myHashFunction(timestamp)+ 
        myHashFunction(data) + nonce*69 + parseInt(previousHash))
        );
};
...
//--------------------------------------------------
    while 
    (parseInt(nextHash, 10) % 100000 !== difficulty)
//--------------------------------------------------
    {
